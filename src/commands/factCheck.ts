...
export async function factCheckCommand(
    context: vscode.ExtensionContext,
    sidebarProvider: SidebarProvider
): Promise<void> {
    const actionName = PromptService.getActionDisplayName(AIAction.FACT_CHECK);

    try {
        logger.info('Fact Check command triggered');
        ...
        vscode.window.showInformationMessage('AI Assistant: Fact Check complete.');

    } catch (error: any) {
        logger.error('Fact Check command failed', error);
